    ...
    <div class="row">
      ...
      <div class="col-sm-6">
        <div class="product-image-large">
          <img src="{{ asset('img/portfolio/' . $projet->id . '.jpg' ) }}" alt="{{ $projet->nom }}">
        </div>
        <div class="colors">
    ...
    <div class="row">
      ...
      <ul class="grid cs-style-2">
        ...
        @foreach($relatedProjets as $id)
          <?php
            $singleProjet = $projetsCtrl->find($id);
            print_r($singleProjet);
            echo "<br/>";
            echo "<br/>";
            echo "<br/>";
            echo "<br/>";
...
<?php /*
        ...
        @foreach($relatedProjets as $relatedProjet)
        <div class="col-md-3 col-sm-6">
          <figure>
            <img src="{{ asset('img/portfolio/' . $similarProjetData->image . '.jpg') }}" alt="img04">
            <figcaption>
              <h3>{{ $similarProjetData->nom }}</h3>
              <span>
                <?php $client = $clientsCtrl->find($similarProjetData->clients_id); ?>
                {{ $client->nom }}
              </span>
              <a href="{{ asset('projet/' . $similarProjetData->id . '/' . $similarProjetData->slug . '.html') }}">Take a look</a>
            </figcaption>
          </figure>
        </div>
        @endforeach
